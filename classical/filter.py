...
def is_similar_line(line1, line2):
    [k1, b1] = line1
    [k2, b2] = line2
    if k1 is None and k2 is None:
        if abs(b1 - b2) < BOUNDARY_OFFSET:
            return True
        else:
            return False
    if k1 == k2:
        dist = abs(b1 - b2) / (1 + k1 ** 2) ** 0.5
        if dist < BOUNDARY_OFFSET:
            return True
        else:
            return False
    return False
...
def vector_negative(vector):
    return Vector(vector.end, vector.start)


def cross_product(vector1, vector2):
    return int(vector1.x) * int(vector2.y) - int(vector2.x) * int(vector1.y)
...
def draw_pools(pools_list):
    ...
    for pool in pools_list:
        ...
        for i, lane in enumerate(pool_lanes):
            drawing = draw_one_rect(drawing, lane, COLOR_RED, CONTOUR_THICKNESS)
            # print(lane)
...
def get_contours(image):
    ...
    element = get_structure_ele(cv.MORPH_RECT, 2)
    morph = cv.morphologyEx(image, cv.MORPH_BLACKHAT, element)

    erosion_element = get_structure_ele(cv.MORPH_RECT, 1)
    ...
    _, erosion_binary = cv.threshold(erosion_gray, 127, 255, cv.THRESH_BINARY)

    return cv.findContours(morph_binary, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)
...
def get_elements(pools_list, model_tag):
    ...
    for k in range(len(layers))[model_tag: upper_limit]:
        layer = layers[k]
        for c_i in layer:
    ...
    # cv.imshow("before remove", draw_pools(pools_list))
    # cv.waitKey(0)
    # normal elements will not be adjacent
    for pool in pools_list:
        elements = pool["elements"]
...
def pre_process(file_name):
    ...
    _, contours, hierarchy = get_contours(input_img)
...
def get_arrows(flows_img):
    ...
    for i, arrow_contour in enumerate(arrow_contours):
        ...
        # print("area:{}, ratio:{}".format(bound[1], "%.3f" % ratio))
        # TODO How to detect an arrow
        if 5 < bound[1] < 70 and 0.5 < ratio < 4.9:
            bound_rec = dilate(bound[0], BOUNDARY_OFFSET)
            # flows_copy = draw_one_rect(flows_copy, bound_rec, COLOR_GREEN, CONTOUR_THICKNESS)
    ...
    for pool in pools:
        ...
        for i, lane in enumerate(lanes):
            ...
            for element in elements_i:
                e_rect = dilate(element, BOUNDARY_OFFSET)
                drawing = truncate(drawing, mask, e_rect)
    return drawing


def remove_text(flows_copy):
    # remove text
    mask = np.zeros_like(input_img, dtype=np.uint8)
    element1 = get_structure_ele(cv.MORPH_RECT, 2)
    morph = cv.morphologyEx(flows_copy, cv.MORPH_BLACKHAT, element1)
    ...
    for i in range(len(del_contours)):
        ...
        if bound[1] > 2:
            flows_copy = truncate(flows_copy, mask, bound_rec)
...
def point_is_near_line(point, line, line_range):
    borders = get_parallel_lines_with_d(line.k, line.b, LINE_AREA_THRESHOLD)
...
def get_discrete_lines_dist_with_same_k_b(line1, line2):
    dist_list = list()
    dist_list.append(get_points_dist(line1.p1, line2.p1))
    dist_list.append(get_points_dist(line1.p1, line2.p2))
    dist_list.append(get_points_dist(line1.p2, line2.p1))
    dist_list.append(get_points_dist(line1.p2, line2.p2))
    return min(dist_list)


def add_discrete_lines(line, discrete_lines):
    exist_lines = discrete_lines[(line.k, line.b)]

    while True:
        merge_index = -1
        for i, one_line in enumerate(exist_lines):
            if get_discrete_lines_dist_with_same_k_b(line, one_line) < 30:
                merge_index = i
                break
        ...
        if merge_index == -1:
            ...
            one_line = exist_lines.pop(merge_index)
            key_index = -1
            if line.k is None:
                key_index = 1
            else:
                key_index = 0
            points = [line.p1, line.p2, one_line.p1, one_line.p2]
            points.sort(key=lambda p: p[key_index])
            line = Line([points[0][0], points[0][1], points[-1][0], points[-1][1]])
    exist_lines.append(line)
    discrete_lines[line.k, line.b] = exist_lines
...
def normalize_all_lines(lines):
    ...
    while len(lines) > 0:
        ...
        k = one_line.k
        b = one_line.b
        borders = get_parallel_lines_with_d(k, b, 5)
        similar = [one_line]
...
def get_element_id(ele_path):
    for i, path in enumerate(all_elements):
        if ele_path[0] == path[0] and ele_path[1] == path[1] and ele_path[2] == path[2]:
            return i
                ...
                if is_overlap(lane, ele_rect):
                    ...
                    for ele_id, element in enumerate(elements_in_lane):
                        if is_overlap(element, ele_rect):
                            ele_path = (pool_id, lane_id, ele_id)
                            return ele_path
...
def get_initial_lines(arrows, line_list):
    ...
    for line in line_list:
        ...
        for i, arrow in enumerate(arrows):
            ...
            # info = get_line_rec_info(line, arrow)
...
def match_arrows_and_elements(arrow_lines, arrows):
    ...
    for i, pool in enumerate(pools):
        ...
        for j in range(len(lanes)):
            elements_in_lane = elements[j]
            for k in range(len(elements_in_lane)):
                all_elements.append((i, j, k))
    # 匹配 arrow 与 element
    # 依据两者位置关系，删除一些线
    # 依据 arrow 确定一些未检出的元素
    arrow_ele_map = dict()
                    ...
                    if is_in(lane, arrow):
                        ...
                        for ele_id, ele in enumerate(elements_in_lane):
                            if is_overlap(dilate_arrow, ele):
                                ...
                                arrow_ele_map[arrow_id] = [(pool_id, lane_id, ele_id), arrow_direct]
                                break
                        if not found:
                            arrow_line = arrow_lines.get(arrow_id)
                            if arrow_line is not None:
...
# def get_end_point_from_arrow_line(key, info, ele_rec):
#     if info[2] == 2:
#         if key.k > 0:
#             # 左上到右下
#             x_list = [info[0], info[1], ele_rec[0], int(ele_rec[1])]
#     elif info[2] == 4:
#     elif info[2] == 1:
#     elif info[2] == 3:
#
...
# def get_intersected_points(line, range, discrete_lines):
#     for discrete_line in discrete_lines:
#         intersection = get_point_of_intersection(line, discrete_line)
        ...
        if len(to_extend_lines) == 0 and len(intersected_lines) == 0:
            ...
            if len(intersected_lines) > 0:
                for intersected_line in intersected_lines:
                    ...
                    if d1 > d2:
                        flow_points.append(intersected_line.p1)
                    else:
                        flow_points.append(intersected_line.p2)
                    detect_one_flow(flow_points, discrete_lines, end_ele_id, flows, arrow_id, merged_lines)
...
def connect_elements(arrows, arrow_lines, arrow_ele_map, discrete_lines):
    ...
    for arrow_id in range(len(arrows)):
        ...
        if arrow_line is not None:
            for key, info in arrow_line.items():
                ...
                detect_one_flow(flow_points, discrete_lines, end_ele_id, flows, arrow_id, merged_lines)
    # print("before:{}".format(len(discrete_lines)))
    for merged_line in merged_lines:
        try:
            ...
            print("remove error")
            continue
    # print("after:{}".format(len(discrete_lines)))
...
def parse_img(f):
    ...
    for discrete_line in discrete_lines:
        cv.line(flows_only, discrete_line.p1, discrete_line.p2, COLOR_RED, CONTOUR_THICKNESS, cv.LINE_AA)
    for arrow in arrows:
        flows_only = draw_one_rect(flows_only, arrow, COLOR_GREEN, CONTOUR_THICKNESS)
    flows_only = draw_lines(flows_only, arrow_lines, COLOR_RED, CONTOUR_THICKNESS)

    flows, discrete_lines = connect_elements(arrows, arrow_lines, arrow_ele_map, discrete_lines)

    for line in discrete_lines:
        cv.line(flows_cp, line.p1, line.p2, COLOR_RED, CONTOUR_THICKNESS, cv.LINE_AA)

    pools_img = draw_pools(pools)
    for arrow_id, arrow_flows in flows.items():
    ...
    for arrow in arrows:
        pools_img = draw_one_rect(pools_img, arrow, COLOR_GREEN, CONTOUR_THICKNESS)
