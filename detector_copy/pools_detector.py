...
def get_pools(input_img, layers, contours_rec, partial_elements):
    ...
    if pools_num == 0:
        ...
        for c_i in potential_pools:
            ...
            # pool_img = helper.draw_one_rect(input_img, pool_bound[0], cfg.COLOR_BLACK, 2)
            # cv.imshow("pool_img", pool_img)
            # cv.waitKey(0)
                ...
                for child_id in c_children:
                    ...
                    if child_id in children_rest:
                        ...
                        if is_pool_header(child_bound):
                            ...
                            if len(pool_lanes) > 0:
                                rest_height = pool_lanes_rect[1] + pool_lanes_rect[3] - lane_y_begin
                                if rest_height > 60:
                                    lane = [pool_lanes_rect[0], lane_y_begin, lane_width, rest_height]
                                    pool_lanes.append(lane)

                                # if rest_height < 10:
                                #     pool_lanes[-1][3] = pool_lanes_rect[1] + pool_lanes_rect[3] - pool_lanes[]
...
def create_default_pool(layer0, contours_rec, pool_dilate_value, layer1=None):
    ...
    if layer1 is None:
        ...
        rest_height = pool_lanes_rect[1] + pool_lanes_rect[3] - next_lane_begin
        if rest_height > 60:
            lane = [left_top_x, next_lane_begin, lane_width, rest_height]
            pool_lanes.append(lane)
...
def get_elements(input_img, layers, contours_rec, partial_elements, pools_list, model_tag):
    ...
    for pool in pools_list:
        ...
        for lane_id in range(len(lanes)):
            lane_elements = elements[lane_id]
            ele_rest = list(range(len(lane_elements)))
            blank = []

            for e_id, ele in enumerate(lane_elements):
                if e_id in ele_rest:
                    roi = helper.truncate(input_img, ele)
                    # cv.imshow("roi", helper.dilate_drawing(roi))
                    # cv.waitKey(0)
                    vertex_sum = sum(roi[0, 0]) + sum(roi[0, -1]) + sum(roi[-1, 0]) + sum(roi[-1, -1])
                    edge_point_sum = sum(roi[0, ele[2] // 2]) + sum(roi[ele[3] // 2, 0]) + sum(
                        roi[-1, ele[2] // 2]) + sum(
                        roi[ele[3] // 2, -1])

                    if vertex_sum == 0 and edge_point_sum == 0:
                        blank.append(ele)
                        ele_rest.remove(e_id)

            # print(blank)

            ele_rest_copy = ele_rest.copy()

            while True:
                adjacent_blank = []
                for e_id in ele_rest:
                    if e_id in ele_rest_copy:
                        ele = lane_elements[e_id]
                        for blank_ele in blank:
                            if helper.is_overlap(ele, helper.dilate(blank_ele, 2)):
                                ele_rest_copy.remove(e_id)
                                blank.append(ele)
                                adjacent_blank.append(e_id)

                                break
                # print(len(adjacent_blank))
                if len(adjacent_blank) == 0:
                    break

            for blank_ele in blank:
                lane_elements.remove(blank_ele)
            ...
            elements[lane_id] = lane_elements
