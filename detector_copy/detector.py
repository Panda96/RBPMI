...
def parse_img(file_path):
    ...
    if len(arrows) > 0:
        ...
        cv.imshow("flows_only", flows_only)
        cv.imwrite("img_output/flows_only.png", flows_only)
        cv.waitKey(0)
...
def run():
    ...
    for im in selected:
        ...
        if os.path.isfile(file_path):
            detect(file_path, classifier, "vgg16_57")
