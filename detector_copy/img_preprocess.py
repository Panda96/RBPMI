...
def get_contours(image, split=True):
    ...
    if split:
        ...
        # morph_0 = cv.morphologyEx(image, cv.MORPH_CLOSE, element)
        # cv.imshow("closing", morph_0)
        # black_hat_0 = morph_0 - image
        # cv.imshow("black_hat_0", black_hat_0)
...
def pre_process(file_path, split=True):
    ...
    contours, hierarchy, partial_elements = get_contours(input_img, split)
...
def main():
    cfg.CONTOUR_THICKNESS = 2
    img_file = "samples/imgs/test/preprocess_0.png"
    # img_file = "samples/imgs/test/parallel.png"
    # cfg.CONTOUR_AREA_THRESHOLD = 1
    get_layers_img(img_file, show=True, output=False, split=True, show_text=False, reverse=False)
